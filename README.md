# 🎮 Arcade Pac-Man 遊戲

一個使用 Python Arcade 函式庫開發的經典 Pac-Man 遊戲，具有智慧 AI 和隨機迷宮生成。

![Python](https://img.shields.io/badge/Python-3.8+-blue.svg)
![Arcade](https://img.shields.io/badge/Arcade-2.6+-green.svg)
![License](https://img.shields.io/badge/License-MIT-yellow.svg)

## 📝 版本更新記錄

### v2.0.0 (Latest) - 多模式系統重構
**重大更新：全新的模式系統架構**

#### 🎯 新增功能
- ✨ **三種遊戲模式**：
  - **Classic Mode（經典模式）**：傳統 Pac-Man 玩法，吃光所有豆子即獲勝
  - **Endless Survival Mode（無盡生存模式）**：豆子與鬼魂會重生，挑戰極限生存時間
  - **Wave/Roguelike Mode（波次模式）**：通過一關進入下一關，難度逐漸提升

- 🎨 **主選單系統**：
  - 美觀的選單介面，使用方向鍵選擇模式
  - 統一的遊戲結束畫面與重啟機制

#### 🏗️ 架構改進
- 📦 **模組化設計**：
  - 新增 `models/` 套件，包含所有遊戲模式
  - `BaseMode` 基礎類別實現共用邏輯
  - 各模式類別繼承並實現特定規則

- 🔧 **程式碼重構**：
  - 主程式（`main.py`）簡化為狀態管理器
  - 模式物件負責各自的遊戲邏輯
  - 更好的職責分離與可維護性

#### 🐛 Bug 修復
- 修正玩家出生點與鬼魂重疊問題
- 改善鬼魂 AI 路徑選擇邏輯
- 優化碰撞檢測效能

#### 📁 檔案結構變更
```
新增：
├── models/                 # 遊戲模式套件
│   ├── __init__.py
│   ├── base_mode.py       # 基礎模式類別
│   ├── classic_mode.py    # 經典模式
│   ├── endless_mode.py    # 無盡模式
│   └── wave_mode.py       # 波次模式
└── menu.py                # 選單系統

修改：
├── main.py                # 簡化為狀態管理器
└── README.md              # 更新說明文件
```

---

### v1.0.0 - 初始版本
- 基本 Pac-Man 遊戲機制
- 隨機迷宮生成系統
- 四種不同個性的鬼魂 AI
- Power Pellet 機制
- 計分系統

---

## ✨ 特色功能

### 🎯 遊戲模式

#### 🎮 Classic Mode（經典模式）
傳統的 Pac-Man 體驗：
- 吃光地圖上所有豆子和 Power Pellets 即獲勝
- 被吃掉的鬼魂不會重生
- 適合喜歡傳統玩法的玩家

#### ⚡ Endless Survival Mode（無盡生存模式）
挑戰你的生存極限：
- 豆子會在 15 秒後自動重生
- Power Pellets 會在 60 秒後重生
- 被吃掉的鬼魂會在 2 秒後重生
- 沒有勝利條件，挑戰最高分數！

#### 🌊 Wave/Roguelike Mode（波次模式）
逐層挑戰，難度遞增：
- 吃光豆子後進入下一波次（Wave）
- 每波次獲得額外獎勵分數（500 × Wave 數）
- 鬼魂速度每波增加 15%
- Power Pellet 效果時間逐漸縮短
- 類 Roguelike 體驗，考驗策略與技巧

### 🎯 遊戲機制
- **隨機迷宮生成**：每次遊戲都有不同的地圖佈局
- **4個智慧鬼魂**：紅、藍、粉、橙，各有獨特AI個性
- **Power Pellets**：吃掉後可以反吃鬼魂（持續10秒）
- **計分系統**：普通豆子+10分，Power Pellet+50分，吃鬼+200分

### 🤖 混合AI系統
每個鬼魂採用先進的混合AI演算法：

- **狀態機**：在追擊、散開、巡邏、隨機漫步4種模式間切換
- **個性化行為**：
  - 👻🔴 紅鬼 (Blinky)：直接追擊玩家
  - 👻💖 粉鬼 (Pinky)：預判玩家移動，埋伏前方 4 格
  - 👻🔵 藍鬼 (Inky)：使用包抄策略
  - 👻🟠 橙鬼 (Clyde)：靠近時逃跑，遠離時追擊
- **噪聲場導航**：生成平滑的隨機移動路徑
- **加權隨機選擇**：70%傾向目標，30%隨機，避免路線固定
- **困住檢測系統**：自動檢測並打破鬼魂的重複路徑

## 🎮 操作說明

### 主選單
| 按鍵 | 功能 |
|------|------|
| ⬆️ 上箭頭 | 向上選擇 |
| ⬇️ 下箭頭 | 向下選擇 |
| ENTER | 確認選擇模式 |

### 遊戲中
| 按鍵 | 功能 |
|------|------|
| ⬆️ 上箭頭 | 向上移動 |
| ⬇️ 下箭頭 | 向下移動 |
| ⬅️ 左箭頭 | 向左移動 |
| ➡️ 右箭頭 | 向右移動 |

### 遊戲結束後
| 按鍵 | 功能 |
|------|------|
| R | 回到主選單 |

## 📦 安裝

### 環境需求
- Python 3.8 或更高版本
- pip 套件管理器

### 安裝步驟

1. **複製儲存庫**
```bash
git clone <your-repo-url>
cd pacman_arcade
```

2. **安裝相依套件**
```bash
pip install -r requirements.txt
```

3. **執行遊戲**
```bash
python main.py
```

## 📁 專案結構

```
pacman_arcade/
├── main.py              # 主遊戲視窗與狀態管理
├── menu.py              # 遊戲主選單系統
├── character.py         # 玩家類別和移動邏輯
├── ghost_ai.py          # 鬼魂AI系統
├── item.py              # 豆子和Power Pellet
├── map_generator.py     # 隨機迷宮生成器
├── constants.py         # 遊戲常數設定
├── requirements.txt     # Python 相依套件
├── README.md            # 專案說明文件
├── models/              # 遊戲模式套件
│   ├── __init__.py
│   ├── base_mode.py     # 基礎模式類別
│   ├── classic_mode.py  # 經典模式
│   ├── endless_mode.py  # 無盡生存模式
│   └── wave_mode.py     # 波次模式
└── assets/              # 遊戲資源檔案
    ├── pacman.png
    ├── ghost_red.png
    ├── ghost_blue.png
    ├── ghost_pink.png
    ├── ghost_orange.png
    └── wall.png
```

## 🎨 技術特點

### 核心技術
- **Python Arcade**：現代化的2D遊戲引擎
- **網格系統**：基於瓦片的移動和碰撞偵測
- **狀態機**：管理遊戲和AI狀態
- **物件導向設計**：模組化的模式系統架構
- **深度優先搜尋**：生成連通的隨機迷宮

### 架構設計
- **BaseMode 基礎類別**：封裝共用的遊戲邏輯
- **模式繼承系統**：各模式只需實現差異化行為
- **事件驅動**：清晰的遊戲狀態流程管理
- **組件化**：玩家、鬼魂、物品各自獨立管理

### 最佳化
- 使用 `arcade.Text` 物件提升渲染效能
- 空間雜湊加速碰撞偵測
- 每幀即時路徑檢測，避免鬼魂卡牆

## 🎯 勝利與失敗條件

### Classic Mode
- **勝利**：吃掉所有普通豆子和Power Pellets
- **失敗**：被鬼魂抓到（非Power Mode狀態）

### Endless Survival Mode
- **勝利**：無（挑戰極限）
- **失敗**：被鬼魂抓到

### Wave/Roguelike Mode
- **勝利**：持續通關，挑戰更高波次
- **失敗**：被鬼魂抓到

## 🔧 設定

可以在 `constants.py` 中調整遊戲參數：

```python
TILE_SIZE = 32                       # 單一地圖格子的像素大小
SCREEN_WIDTH = 19 * TILE_SIZE        # 螢幕寬度（19 格）
SCREEN_HEIGHT = 21 * TILE_SIZE       # 螢幕高度（21 格）
PLAYER_SPEED = 2.0                   # 玩家移動速度（每幀位移）
GHOST_SPEED = 1.0                    # 鬼魂移動速度，略慢於玩家
COLOR_BG = (0, 0, 0)                 # 背景顏色（黑色）
```

### Endless Mode 設定
可在 `models/endless_mode.py` 中調整重生時間：
```python
PELLET_RESPAWN_FRAMES = 15 * 60   # 豆子重生時間（秒 × 60）
POWER_RESPAWN_FRAMES = 60 * 60    # Power Pellet 重生時間
GHOST_RESPAWN_FRAMES = 2 * 60     # 鬼魂重生時間
```

### Wave Mode 設定
可在 `models/wave_mode.py` 中調整難度曲線：
```python
speed_factor = 1.0 + 0.15 * (self.wave - 1)  # 每波速度增加 15%
```

## 🎮 遊戲特性與設計

### 隨機性與多樣性
- **動態地圖生成**：每次遊戲都會生成獨特的迷宮佈局
- **鬼魂行為多樣化**：4種AI模式（追擊、散開、巡邏、隨機漫步）動態切換
- **困住檢測系統**：自動檢測並打破鬼魂的重複路徑
- **模式多樣性**：三種不同玩法滿足不同需求

### 地圖結構影響
由於採用完全隨機的迷宮生成，某些地圖可能會出現以下特性：
- **區域隔離**：部分鬼魂可能暫時被困在地圖的某個區域
- **難度變化**：不同地圖的鬼魂威脅程度不同，增加遊戲的挑戰性和重玩價值
- **策略性**：利用地形優勢躲避或引誘鬼魂

> 💡 **提示**：如果覺得當前地圖太簡單或太難，回到主選單重新開始遊戲即可重新生成地圖！每次都是全新的體驗。


## 📄 授權條款

本專案採用 MIT 授權條款。詳見 LICENSE 檔案。

## 🙏 致謝

- 靈感來源：經典 Pac-Man 遊戲與 Google 萬聖節小遊戲
- 使用框架：[Python Arcade Library](https://api.arcade.academy/)

## 📧 聯絡方式

如有問題或建議，歡迎提交 Issue 或 Pull Request！

---

**Enjoy the game! 🎮✨**